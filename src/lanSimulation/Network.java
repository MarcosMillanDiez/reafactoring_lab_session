...
public class Network {
	...
	public boolean hasWorkstation(String ws) {
		...
		if (n == null) {
			...
			return n.oldType == NodeType.WORKSTATION;
		}
	};
	...
	public boolean consistentNetwork() {
		...
		while (iter.hasMoreElements()) {
			...
			if (currentNode.oldType != NodeType.WORKSTATION) {
				return false;
			}
			;
		}
		...
		while (!encountered.containsKey(currentNode.name_)) {
			...
			if (currentNode.oldType == NodeType.PRINTER) {
				printersFound++;
			}
	...
	public boolean requestBroadcast(Writer report) {
		...
		try {
			String a = ">>> Broadcast travelled whole token ring.\n\n";
			report.write(a);
		} catch (IOException exc) {
			// just ignore
		}
	...
	public boolean requestWorkstationPrintsDocument(String workstation, String document, String printer,
		...
		if (packet.atDestination(currentNode)) {
			...
			try {
				String a = ">>> Destinition not found, print job cancelled.\n\n";
				report.write(a);
				report.flush();
			} catch (IOException exc) {
				// just ignore
	...
	public void printAuthorTitle(Writer report, String author, String title, String info) throws IOException {
...
//	public void appendElement(StringBuffer buf, Node currentNode, String tag1, String tag2) {
//		buf.append(tag1);
